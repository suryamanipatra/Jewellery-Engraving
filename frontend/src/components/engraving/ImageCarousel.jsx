...
const ImageCarousel = ({
  ...
  const handleImageSelection = (visibleIndex) => {
      ...
      <div className="grid grid-cols-2 gap-2 md:gap-4 min-h-[180px] md:min-h-[260px]">
        ...
        {visibleImages.length > 0 ? (
          ...
          visibleImages.map((url, index) => (
            <div
              key={index}
              className={`w-[80px] h-[80px] md:w-[120px] md:h-[120px] flex items-center justify-center shadow-md overflow-hidden rounded-xl md:rounded-3xl cursor-pointer border-2 ${selectedImage === url ? "border-blue-500" : "border-transparent"
                }`}
