...
export const generatePdfAndSendMail = async (myImage, selectedImage, modifiedImages, setShowLoader, setMessage, setError, email, role = '') => {
    ...
    if (role) {
        ...
        tableData = selectedImage?.engraving_details?.engraving_lines
            .flatMap(line => JSON.parse(line.product_details || "[]").map(pd => [pd.property, pd.value]));
        tableData = Array.from(new Set(tableData.map(JSON.stringify)), JSON.parse);
